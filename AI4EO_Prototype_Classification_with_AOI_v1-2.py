...
for x in li:
    if x=="raspre":
        ...
        atmcorr_raspre=atmospheric_correction(in_directory, out_directory, sen2cor_path, delete_unprocessed_image=False)
    if x=="raspos":
        tempvar=fpos
        in_directory=fpos
        out_directory=os.path.dirname(tmpdir)+"/"+"L2"
        ensure_dir(out_directory+"/")
        atmcorr_raspos=atmospheric_correction(in_directory, out_directory, sen2cor_path, delete_unprocessed_image=False)
...
ensure_dir(out_dir+"/")
multi_cmask=pyeo.raster_manipulation.preprocess_sen2_images(l2_dir, out_dir, l1_dir, cloud_threshold=60, buffer_size=0, epsg=None, bands=('B04','B08', 'B12'), out_resolution=10)
